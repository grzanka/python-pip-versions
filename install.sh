...
case `uname` in
  Linux )
     which dnf && { dnf_install $PYTHON_VERSION; return; }         # Fedora
     which yum && { yum_install $PYTHON_VERSION; return; }         # CentOS
     which zypper && { zypper_install $PYTHON_VERSION; return; }   # ????
     which apt-get && { apt_install $PYTHON_VERSION; return; }     # Ubuntu, Debian
     ;;
  * )
